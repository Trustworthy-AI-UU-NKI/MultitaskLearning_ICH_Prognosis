...
for fold, (train_index, test_index) in enumerate(skf.split(images_all, labels_all)):
    ...
    path_to_save_model=os.path.join(path_to_save_model_dir,
                                                "DenseNet_"+name_file_model+"_fold"+str(fold)+".pth")
    if not os.path.isfile(path_to_save_model):
        model = PrognosisICH_ThreeClassGCSBinaryAge_Model(image_shape=image_shape, depth=depth, spatial_dims=3, in_channels=1, num_classes_binary=1, dropout_prob=0.2)
        ...
        binary_loss_function = torch.nn.BCEWithLogitsLoss().to(device)
        ...
        for epoch in range(max_epochs):
            ...
            for batch_data in train_loader:
                ...
                # print("Binary loss prognosis:", binary_loss)
                # print("Ordinal loss GCS:", ordinal_loss_gcs)
                # print("Ordinal loss Age:", ordinal_loss_age)
                combined_loss = 0.4*binary_loss + 0.3*ordinal_loss_gcs + 0.3*ordinal_loss_age
            ...
            if (epoch + 1) % val_interval == 0:
                ...
                for val_data in val_loader:
                    ...
                    with torch.no_grad():
                        ...
                        # for prognosis
                        binary_val_outputs = binary_val_outputs.squeeze()
                ...
                print("Predictions count in validation in prognosis:", dict(zip(unique, counts)))
                print("Probabilities in validation (prognosis):", all_probabilities)
                # Calculate metrics
                ...
                print("Predictions count validation Age:", dict(zip(unique_age, counts_age)))
        ...
        plt.savefig(os.path.join(path_to_save_results, "TrainAndVal_LossAUC_ProgAge_"+name_file+"_fold"+str(fold)+".png"))
        plt.close()
        ...
        plt.xlabel("epoch")
        plt.legend(['train_GCS', 'val_GCS'], loc='best')
        plt.savefig(os.path.join(path_to_save_results, "TrainAndVal_GCS_metrics"+name_file+"_fold"+str(fold)+".png"))
        plt.close()
    ...
    combined=np.column_stack((fold_array, labels_gcs_test_tensor, predicted_labels_gcs_test,
                              labels_test_tensor, all_probabilities_test, predicted_labels_test,
    ...
    fold_metrics_df = pd.concat([fold_metrics_df, pd.DataFrame([[fold, test_gcs_balanced_accuracy, 
                                                                 ...
                                                                 test_gcs_uoc_index, test_gcs_kappa,
                                                                 test_auc, test_balanced_accuracy, test_accuracy,
                                                                 test_specificity, test_NPV, test_precision, test_recall, test_f1,
                                                                 test_age_auc, test_age_balanced_accuracy, test_age_accuracy, 
                                                                 test_age_specificity, test_age_NPV, test_age_precision,
                                                                 test_age_recall, test_age_f1]], columns=fold_metrics_df.columns)], ignore_index=True)
