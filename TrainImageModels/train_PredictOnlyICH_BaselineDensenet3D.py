...
for fold, (train_index, test_index) in enumerate(skf.split(images_all, labels_all)):
    ...
    if not os.path.isfile(path_to_save_model):
        ...
        accuracy_values = []
        max_epochs = 100
        path_to_save_model=os.path.join("/home/ubuntu/tenerife/data/ICH_models/ICH_3DMonaiClassificationDenseNet",
                                                    "PrognosisModelICH_DenseNet_SkullStripping_SmallDataAug_fold"+str(fold)+".pth")
        ...
        plt.xlabel("epoch")
        plt.legend(['train', 'val'], loc='best')
        plt.savefig(os.path.join("/home/ubuntu/tenerife/data/ICH_results/3DClassificationImageModelMonai", "TrainAndVal_DenseNet_SkullStripping_SmallDataAug_fold"+str(fold)+".png"))
        plt.close()
    ...
    evaluator = EvaluateThresholds(all_probabilities_test, labels_test_tensor,
             path_to_save_auc_plot=os.path.join("/home/ubuntu/tenerife/data/ICH_results/3DClassificationImageModelMonai/AUC_plots",
                                                 "AUC_testplot_DenseNet_SkullStripping_SmallDataAug_fold"+str(fold)+".png"), fold=fold)
