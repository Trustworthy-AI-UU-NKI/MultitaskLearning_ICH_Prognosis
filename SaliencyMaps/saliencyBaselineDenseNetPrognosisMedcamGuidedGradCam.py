...
for fold, (train_index, test_index) in enumerate(skf.split(images_all, labels_all)):
    ...
    model = medcam.inject(model, backend='ggcam', layer='features.transition3.conv', output_dir=path_to_save_saliency_maps, save_maps=True, return_score=False, cudnn=True)
